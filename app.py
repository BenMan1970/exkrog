...
st.set_page_config(page_title="Scanner Confluence Forex (Exchange Rates API Data)", page_icon="⭐", layout="wide")
st.title("🔍 Scanner Confluence Forex Premium (Données Exchange Rates API)")
st.markdown("*Utilisation de l'API Exchange Rates pour les données de marché H1*")
...
def get_data_exchangerates(symbol: str, timeframe: str = 'H1', period_days: int = 15):
    print(f"\n--- Début get_data_exchangerates: sym='{symbol}', timeframe='{timeframe}', period='{period_days}d' ---")
    try:
        # Initialiser l'API Exchange Rates avec la clé API
        api_key = st.secrets["exchangerates"]["API_KEY"]
        ...
        # Construire la paire de devises (base et cible)
        base_currency = symbol[:3]  # Ex: EUR de EURUSD
        target_currency = symbol[3:]  # Ex: USD de EURUSD

        # Faire une requête pour les données historiques
        params = {
            "access_key": api_key,
        ...
        if not data.get("success", False):
            st.warning(f"Exchange Rates API: Erreur pour {symbol} - {data.get('error', 'Erreur inconnue')}")
            print(f"Exchange Rates API: Erreur pour {symbol} - {data.get('error', 'Erreur inconnue')}")
            return None

        # Extraire les taux
        rates = data.get("rates", {})
        if not rates:
            st.warning(f"Exchange Rates API: Données insuffisantes ou vides pour {symbol}.")
            print(f"Exchange Rates API: Données insuffisantes ou vides pour {symbol}.")
            return None

        # Construire un DataFrame avec les données (simuler OHLC)
        dates = []
        closes = []
        for date, rate in rates.items():
            rate_value = rate.get(target_currency)
            if rate_value is not None:
                dates.append(date)
                closes.append(rate_value)

        if len(closes) < 50:
            st.warning(f"Exchange Rates API: Données insuffisantes pour {symbol} ({len(closes)} points).")
            print(f"Exchange Rates API: Données insuffisantes pour {symbol} ({len(closes)} points).")
            return None
        ...
        # Simuler OHLC (approximation car l'API fournit des taux journaliers)
        df["Open"] = df["Close"].shift(1).fillna(df["Close"])
        df["High"] = df["Close"].rolling(window=2).max().fillna(df["Close"])
        df["Low"] = df["Close"].rolling(window=2).min().fillna(df["Close"])

        df = df[["Open", "High", "Low", "Close"]]
        print(f"Données pour {symbol} OK. Retour de {len(df)} lignes.\n--- Fin get_data_exchangerates {symbol} ---\n")
        return df

    except Exception as e:
        st.error(f"Erreur Exchange Rates API pour {symbol}: {type(e).__name__} - {e}")
        print(f"ERREUR Exchange Rates API pour {symbol}:\n{traceback.format_exc()}")
        return None
...
with col1:
    ...
    pair_to_debug = st.selectbox("🔍 Afficher données pour:", ["Aucune"] + PAIRS_EXCHANGE, index=0)
    scan_btn = st.button("🔍 Scanner (Données Exchange Rates H1)", type="primary", use_container_width=True)
with col2:
    if scan_btn:
        st.info(f"🔄 Scan en cours (Exchange Rates H1)...")
        pr_res = []
        pb = st.progress(0)
        stx = st.empty()
        if pair_to_debug != "Aucune":
            st.subheader(f"Données pour {pair_to_debug} (Exchange Rates):")
            debug_data = get_data_exchangerates(pair_to_debug, timeframe="H1", period_days=5)
            ...
            st.divider()
        for i, symbol_scan in enumerate(PAIRS_EXCHANGE):
            pnd = symbol_scan[:3] + "/" + symbol_scan[3:]  # Ex: EUR/USD
            cp = (i + 1) / len(PAIRS_EXCHANGE)
            pb.progress(cp)
            stx.text(f"Analyse (Exchange Rates H1): {pnd} ({i+1}/{len(PAIRS_EXCHANGE)})")
            d_h1_exchange = get_data_exchangerates(symbol_scan, timeframe="H1", period_days=15)
            if d_h1_exchange is not None:
                sigs = calculate_all_signals_pine(d_h1_exchange)
                if sigs:
                    ...
                    pr_res.append({
                        ...
                        'details': {'Info': 'Calcul signaux (Exchange Rates) échoué'}
                    })
            else:
                pr_res.append({
                    'Paire': pnd, 'Direction': 'ERREUR DONNÉES EXCHANGE RATES', 'Conf. (0-6)': 0, 'Étoiles': 'N/A',
                    'RSI': 'N/A', 'ADX': 'N/A', 'Bull': 0, 'Bear': 0,
                    'details': {'Info': 'Données Exchange Rates non dispo/symb invalide (logs serveur)'}
                })
        ...
        if pr_res:
            ...
            if not show_all:
                st.success(f"🎯 {len(dfd)} paire(s) avec {min_conf}+ confluence (Exchange Rates).")
            else:
                st.info(f"🔍 Affichage des {len(dfd)} paires (Exchange Rates).")
            if not dfd.empty:
                ...
                with st.expander("📊 Détails des signaux (Exchange Rates)"):
                ...
                st.warning(f"❌ Aucune paire avec critères filtrage (Exchange Rates). Vérifiez erreurs données/symbole.")
        else:
            st.error("❌ Aucune paire traitée (Exchange Rates). Vérifiez logs serveur.")
with st.expander("ℹ️ Comment ça marche (Logique Pine Script avec Données Exchange Rates)"):
    st.markdown("""**6 Signaux Confluence:** HMA(20), RSI(10), ADX(14)>=20, HA(Simple), SHA(10,10), Ichi(9,26,52).**Comptage & Étoiles:** Pine.**Source:** Exchange Rates API.""")
st.caption("Scanner H1 (Exchange Rates). Multi-TF non actif.")
